...
import { ConnectCalendarButton } from '@/features/calendar/components/connect-calendar-button'
import { DisconnectCalendarButton } from '@/features/calendar/components/disconnect-calendar-button'
import { SyncCalendarButton } from '@/features/calendar/components/sync-calendar-button'
...
import { ExtensionTokenSection } from '@/features/settings/components/extension-token-section'
import { LlmProviderSection } from '@/features/settings/components/llm-provider-section'
...
export default async function SettingsPage() {
  ...
  const settingsUserResult = await withRls(userId, async (tx) =>
    tx.user.findUnique({
      where: { id: Number(userId) },
      select: {
        gmailRefreshTokenEncrypted: true,
        gmailLastSyncAt: true,
        calendarRefreshTokenEncrypted: true,
        calendarLastSyncAt: true,
        extensionApiTokenHash: true,
        classifierLlmProvider: true,
      },
    }),
  )
  const user = settingsUserResult.isOk() ? settingsUserResult.value : null

  const isConnected = !!user?.gmailRefreshTokenEncrypted
  const isCalendarConnected = !!user?.calendarRefreshTokenEncrypted
  const hasExtensionToken = !!user?.extensionApiTokenHash
  const classifierLlmProvider = user?.classifierLlmProvider ?? env.CLASSIFIER_LLM_PROVIDER
    ...
    <AppShell>
      <div className="mx-auto max-w-3xl px-6 py-10 lg:px-10 lg:py-14">
        ...
        <div className="space-y-6">
          ...
          <section className="rounded-lg border border-border bg-card p-6">
            <div className="space-y-1">
              <h2 className="text-xl font-medium">{t('calendar.title')}</h2>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3">
              ...
              >
                <span
                  ...
                  aria-hidden="true"
                />
                {isCalendarConnected ? t('calendar.connected') : t('calendar.disconnected')}
              </span>
            ...
            {user?.calendarLastSyncAt && (
              <p className="mt-3 font-mono text-xs text-muted-foreground">
                {t('calendar.lastSynced', { date: user.calendarLastSyncAt.toLocaleString() })}
              </p>
            )}
          </section>
          ...
          <ExtensionTokenSection hasToken={hasExtensionToken} />

          <LlmProviderSection
            provider={classifierLlmProvider}
